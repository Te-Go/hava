...
export const fromSlug = (slug: string): string => {
  ...
  // Defensive Bypass Guard: If string already contains raw Turkish diacritics or capitalization, return as-is
  ...
  if (sanitized.includes('-')) {
    ...
    if (parts.length === 2 && TURKISH_NAMES[parts[1]]) return TURKISH_NAMES[parts[1]];
    if (parts.length === 2 && TURKISH_NAMES[parts[0]] && TURKISH_NAMES[parts[1]]) return TURKISH_NAMES[parts[1]];
  }

  if (TURKISH_NAMES[sanitized]) return TURKISH_NAMES[sanitized];
...
export const getWeatherData = async (city: string, revalidateSecs: number = 900): Promise<WeatherData> => {
  ...
  // Next.js Route Caching & Fallback Proxy Model
  if (typeof window === 'undefined') {
    console.log(`🖥️ [Server-Side ISR] Fetching direct Open-Meteo API for "${finalCityName}"`);
  } else {
    // Client-side: query Next.js API Route for cached traffic/weather
    try {
      let coords = resolvedCoords || { lat: 39.0, lon: 35.5 };
      const proxyUrl = `/api/weather?lat=${coords.lat}&lon=${coords.lon}`;
      const response = await fetch(proxyUrl);
      if (!response.ok) throw new Error('Next.js API proxy failed');

      const data = await response.json();
      console.log(`✓ Next.js client-side proxy fetch success for ${finalCityName}`);
      return mapOpenMeteoToModel(finalCityName, data);
    } catch (e) {
      console.warn("Next.js API proxy failed, trying direct Open-Meteo...", e);
    }
  }
  ...
  try {
    ...
    const response = await fetch(url, { next: { revalidate: revalidateSecs } });
    if (!response.ok) throw new Error('Open-Meteo API Failed');
